...
async def save_messages_from_langgraph_state(
    ...
    run_id: str | None = None,
    request_id: str | None = None,
    agent_state: AgentStatePayload | dict | None = None,
) -> None:
    ...
    for msg in messages:
        ...
        if msg_type == "ai":
            if isinstance(agent_state, dict):
                msg_dict = {
                    **msg_dict,
                    "artifacts": agent_state.get("artifacts") or [],
                    "files": agent_state.get("files") or {},
                }
                if isinstance(agent_state.get("token_usage"), dict):
                    msg_dict["token_usage"] = dict(agent_state["token_usage"])
