...
async def finalize_intake(
    *, db: AsyncSession, intake: IntakeResult, commit: bool = True, enqueue: bool = True
) -> None:
    ...
    await finalize_dispatch(db=db, dispatch=dispatch, commit=commit, enqueue=enqueue)


async def finalize_dispatch(
    *, db: AsyncSession, dispatch: DispatchResult | None, commit: bool = True, enqueue: bool = True
) -> None:
    """提交当前事务；提交成功后才把已创建的 run 投递给 ARQ。"""
    if enqueue and not commit:
        raise ValueError("未提交数据库事务前不能投递 AgentRun")
    if commit:
        await db.commit()
    if dispatch and enqueue:
...
def _build_message_metadata(
    *, request_id: str, source: str, input_message: AgentRunInputMessage, meta: dict
    ...
    for key in ("knowledge_base_ids", "skill_slugs", "mcp_server_slugs"):
        value = meta.get(key)
        if isinstance(value, list):
            metadata[key] = [str(item).strip() for item in value if str(item).strip()]
    if isinstance(meta.get("output_config"), dict):
        metadata["output_config"] = dict(meta["output_config"])
    for key in ("schedule_id", "scheduled_run_id"):
        value = meta.get(key)
        if value:
