...
async def test_save_messages_from_langgraph_state_backfills_run_output_message(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    await svc.save_messages_from_langgraph_state(
        ...
        request_id="req-1",
        agent_state={
            "artifacts": [{"path": "outputs/report.md", "mime_type": "text/markdown"}],
            "files": {"report.md": "outputs/report.md"},
    ...
    assert conv_repo.saved_messages[0]["extra_metadata"]["artifacts"] == [
        {"path": "outputs/report.md", "mime_type": "text/markdown"}
    ]
    assert conv_repo.saved_messages[0]["extra_metadata"]["token_usage"] == {
        "input_tokens": 12,
        "output_tokens": 34,
    }
