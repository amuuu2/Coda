...
# Coda Initialization Script for Bash/Linux/macOS
...
ensure_required_api_env() {
    if grep -Eq '^(DASHSCOPE_API_KEY|SILICONFLOW_API_KEY)=.+' .env; then
        return
    fi

    echo "A model provider API key is missing in .env."
    echo "1) Alibaba Bailian (DASHSCOPE_API_KEY)"
    echo "2) SiliconFlow (SILICONFLOW_API_KEY)"
    while true; do
        read -p "Please select a provider (1 or 2): " PROVIDER_CHOICE
    ...
    if [ "$PROVIDER_CHOICE" = "1" ]; then
        MODEL_API_KEY_NAME="DASHSCOPE_API_KEY"
    else
        MODEL_API_KEY_NAME="SILICONFLOW_API_KEY"
    fi

    while true; do
        read -s -p "Please enter your ${MODEL_API_KEY_NAME}: " MODEL_API_KEY
        echo ""
    ...
    set_env_value "$MODEL_API_KEY_NAME" "$MODEL_API_KEY"
}
...
echo "🚀 Initializing Coda project..."
echo "=================================="
...
if [ -f ".env" ]; then
    ...
    echo "🔑 Model provider API Key required"
    echo "1) Alibaba Bailian (DASHSCOPE_API_KEY)"
    echo "2) SiliconFlow (SILICONFLOW_API_KEY)"
    while true; do
        read -p "Please select a provider (1 or 2): " PROVIDER_CHOICE
...
${MODEL_API_KEY_NAME}=${MODEL_API_KEY}
