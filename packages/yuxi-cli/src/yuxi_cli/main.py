...
console = Console()
app = typer.Typer(help="Coda command line client.", invoke_without_command=True)
remote_app = typer.Typer(help="Manage Coda remotes.")
agent_app = typer.Typer(help="Run and manage Coda agents.")
...
def _print_remote_context(store: ConfigStore, remote_name: str | None) -> None:
    ...
    console.print(f"Coda CLI {__version__}")
...
def chat(
    ...
    agent_slug: str = typer.Option(
        "default-chatbot", "--agent-slug", help="Coda agent slug."
    ),
...
def eval_agent(
    ...
    agent_slug: str = typer.Option(..., "--agent-slug", help="Coda agent slug."),
    ...
    timeout_seconds: float = typer.Option(900, "--timeout-seconds", help="Per item Coda API timeout."),
):
