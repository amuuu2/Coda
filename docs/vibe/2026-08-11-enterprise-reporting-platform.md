...
本轮补充数据分析工作台的空状态交互，与自动化中心、抽取中心保持一致：页面进入时只展示数据源列表和查询历史；点击“新建数据源”或已有数据源后才展示数据源配置；未选中数据源时不展示查询输入表单，选中后再展示提问、结果和 Schema 区域。自动化任务统一使用 `Asia/Shanghai`，不再提供时区输入框。

验收 checklist：

- [x] 数据分析页面初次进入不自动打开数据源配置表单。
- [x] 点击“新建数据源”后显示配置表单，已有数据源点击后进入编辑。
- [x] 未选择数据源时不显示查询输入框，选择后显示查询与 Schema。
- [x] 自动化中心不显示时区输入框，创建、编辑和列表均使用上海时区。
- [x] 自动化中心、抽取中心和数据分析页面的创建表单均由用户操作触发。
