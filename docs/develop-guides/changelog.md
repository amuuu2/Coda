...
- 重设计前端首页、登录页及主要后台工作流：统一聊天、智能体与模型管理、质量闭环、工作区、知识库、技能、工具、MCP 和数据总览的页面骨架、工具栏、卡片、列表、空状态与交互控件；保留既有业务逻辑、权限、路由和 API，不新增依赖。
...
- 新增 Agent 质量评测与自优化闭环：按准备样本、创建候选、运行对比三步评测 Prompt、模型和 Skill，清晰展示分数与逐题回答，经人工批准后发布并支持回滚；评测对话不进入普通会话，实验历史可删除；新增质量表 Migration、API 和未运行测试，无新增依赖。
