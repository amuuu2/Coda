...
- 统一对外产品版本与构建标识：后端启动日志显示 `Coda v0.1.1`，Docker 镜像改为 `coda-api`、`coda-web` 和 `coda-sandbox-provisioner`，前后端项目版本及 CLI 服务端兼容下限同步为 `0.1.1`；保留 `yuxi` Python 包名、`YUXI_*` 环境变量、数据库默认名和数据卷路径以兼容现有部署。

- 新增企业智能报告平台静态实现：自动化中心复用 AgentRun/ARQ 支持 Cron、运行历史和幂等派发；抽取中心支持 JSON Schema 校验、证据、人工修订、真实重试次数和 JSON/CSV 导出；数据分析工作台支持 MySQL/PostgreSQL 数据源、Schema 白名单、AST SQL 安全门禁、只读查询和审计。新增 `croniter`、`jsonschema`、`sqlglot` 依赖，Migration、测试和目标环境清单已补充；取消后的自动化任务不可重新启用，当前仅完成代码静态实现，未运行。

::: warning 升级提醒
升级到 v0.7.2 后，管理员此前创建的 stdio MCP 会被禁用，也无法重新启用。请在详情页迁移为 SSE 或 Streamable HTTP，或直接删除；代码内置的系统 stdio MCP 不受影响。
:::

- 项目展示品牌统一为 Coda：更新界面、文档、默认配置与仓库链接，移除原作者联系方式和推广入口；保留 `yuxi` 包名、CLI 命令、`.yuxi` 配置、`YUXI_*` 环境变量及 `X-Yuxi-*` 协议头，避免破坏现有部署与集成。
...
- 优化 Agent 上下文压缩：Coda 的 DeepAgents summary adapter 在生成 summary 与写入 conversation history 时，会先对本次模型调用的临时消息视图执行 L1 结构精简，截断旧 `write_file`/`edit_file` 大参数，并把超过阈值的大 `ToolMessage.content` 写入 `outputs/large_tool_results` 后替换为路径和有限预览；L1 不修改 LangGraph state 原始消息，L1 后若上下文低于入口阈值的 40% 则直接调用模型，不生成 summary event，仍超过时才进入 L2 summary。L2 继续使用 DeepAgents `_summarization_event.cutoff_index` 重建 effective messages；Summary 阈值判断改为使用 Coda 自己的近似 token 计算结果，不再根据 provider `usage_metadata.total_tokens` 或 usage scaling 提前触发；首次写入 `conversation_history` 前读取旧文件的 sandbox 404 会按 `file_not_found` 处理，不再产生误导性 warning；`present_artifacts` 会拒绝展示 `large_tool_results` 与 `conversation_history` 等工具调用阶段文件。新增管理员可配置项 `summary_keep_messages`、`summary_prompt`、`summary_tool_result_token_limit` 与 `max_execution_steps`，分别控制摘要后保留消息数、摘要提示词、summary 阶段工具结果预览上限和 LangGraph `recursion_limit`。
...
- 新增外部系统 Agent 调用入口：独立 `agent-invocation` router 提供 `POST /api/agent-invocation/agent-call/runs` 与 `POST /api/agent-invocation/agent-call/runs/result`，字段沿用 Coda 命名（`agent_slug/thread_id/request_id/model_spec`），复用 AgentRun 队列和结果读取能力；支持非流式同步等待或 `async_mode=true` 立即返回 `run_id`，Agent Call 不允许通过 `agent_call_meta.context` 覆盖 Agent context，运行时模型覆盖只允许走独立 `model_spec`；修复无 `thread_id` 且模型校验失败时提前提交空对话，导致孤儿对话和 `request_id` 失败重试非幂等的问题；Agent Call 的 `messages[].content` 兼容 OpenAI 风格的 `text`/`image_url` 多模态数组，纯文本数组不再误报 422，图片输入会保留原始 LangChain 多模态消息供 AgentRun worker 恢复；Agent Eval 与 Agent Call 统一通过 conversation-backed invocation helper 创建 run，后续定时任务等入口只需做请求解析和结果出口适配。
...
- 修复子智能体流式事件兼容：Coda task middleware 的 DeepAgents 子智能体 transformer 改用专用 `yuxi_subagents` projection，避免与 LangChain `create_agent` 默认注册的 `subagents` projection 冲突导致运行流式消息时报错；子线程路由收集优先读取 Coda projection，并保留原 `subagents` fallback。
...
- 补充 Langfuse 集成方案文档：明确采用“云端优先、先 tracing 后 feedback”的接入路径，并约定 Coda 的 `user/thread` 到 Langfuse `user_id/session_id` 的映射关系
...
- 增加企业智能报告与数据分析平台的实施方案、目标部署清单，以及自动化任务、结构化文档抽取和受控数据分析的 MVP 设计记录。

### Changed

- 优化企业工作台交互：Agent 创建时可直接配置模型和系统提示词；自动化任务固定使用上海时区并隐藏时区输入；自动化、抽取和数据分析页面的创建表单改为点击后显示，数据分析页面未选数据源时不再展示查询表单。
