...
感谢你对 Coda 的关注。我们欢迎 Bug 修复、功能改进、测试补充、文档更新以及其他能够让项目变得更好的贡献。
...
- 搜索已有 [Issues](https://github.com/amuuu2/Coda/issues)，避免重复提交相同问题。
- 如果任务来自 GitHub Project，阅读任务描述、关联 Issue、验收标准和已有讨论，并确认任务已经分配给你。
- 对影响范围较大、需求边界不明确或会改变现有架构的改动，先通过 Issue 或 [Discussions](https://github.com/amuuu2/Coda/discussions) 对齐方案。
- 一个 PR 只解决一个明确问题，不混入无关重构、格式化或“顺手优化”。

修改不熟悉的模块前，请先阅读 [ARCHITECTURE.md](https://github.com/amuuu2/Coda/blob/main/ARCHITECTURE.md)，了解前后端边界、主要运行链路和架构不变量，再通过代码搜索定位具体实现。
...
7. 从 Fork 分支向 `amuuu2/Coda:main` 发起 PR。
...
先在 GitHub 上 Fork [amuuu2/Coda](https://github.com/amuuu2/Coda)，然后克隆自己的 Fork：
...
```bash
git remote add upstream https://github.com/amuuu2/Coda.git
git remote -v
```
...
upstream  amuuu2/Coda，用于获取主仓库更新
```
...
Coda 使用 Docker Compose 管理开发环境。开发、调试和测试应在运行中的容器环境中完成。
...
3. **风格一致且位置合理**：新代码与相邻实现保持一致，Python 代码符合 Python 3.12+ 和 Pythonic 风格；路由、服务、仓储、前端 API、测试等内容位于正确边界，并符合 `AGENTS.md`、[ARCHITECTURE.md](https://github.com/amuuu2/Coda/blob/main/ARCHITECTURE.md)、[测试规范](./testing-guidelines.md) 和相关开发文档。
...
```text
base repository: amuuu2/Coda
base branch:     main

head repository: <your-username>/Coda
compare branch:  当前任务分支
```

不要向自己 Fork 的 `main` 创建 PR，也不要把多个任务分支合并后再统一提交。

PR 标题应直接表达目标，正文按照 [PR 模板](https://github.com/amuuu2/Coda/blob/main/.github/PULL_REQUEST_TEMPLATE.md) 填写，并至少包含：
...
- Bug 反馈：[GitHub Issues](https://github.com/amuuu2/Coda/issues)
- 功能和方案讨论：[GitHub Discussions](https://github.com/amuuu2/Coda/discussions)
