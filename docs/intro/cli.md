...
`yuxi-cli` 是 Coda 的命令行客户端，适合在本地脚本或终端中管理远程实例、登录账号、上传知识库文件，以及运行部分智能体任务。
...
先添加一个 Coda 实例地址，再设为当前默认 remote：
...
如果已经在 Coda 中创建了 API Key，也可以直接导入：
...
该命令会读取 Langfuse 数据集输入，调用 Coda 智能体运行，并把结果回传到对应实验中。
